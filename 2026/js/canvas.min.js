...
function setup() {
    setupSymbols(), addLetters(), resize(), mouse = addMouseInteraction(), two.bind("resize", resize), window.addEventListener("deviceorientation", function(e) {
    ...
    }, !1), two.playing = !0, Matter.Events.on(solver, "afterUpdate", update), startFixedStepLoop();
    var footerEl = document.querySelector("footer");
    if (footerEl && "undefined" != typeof ResizeObserver) new ResizeObserver(resize).observe(footerEl);
}
...
function startFixedStepLoop() {
    ...
    function frame(time) {
        if (null !== lastTime) accumulator += Math.min(time - lastTime, 250);
        lastTime = time;
        try {
...
function resize() {
    var e = bounds.thickness
        , t = 1.25 * two.width
        , i = 1.25 * two.height;
    vector.x = two.width / 2 - two.width / 2 - e / 2, vector.y = two.height / 2, Matter.Body.setPosition(bounds.left.entity, vector), Matter.Body.scale(bounds.left.entity, 1 / bounds.left.entity.scale.x, 1 / bounds.left.entity.scale.y), Matter.Body.scale(bounds.left.entity, e, i), bounds.left.entity.scale.x = e, bounds.left.entity.scale.y = i, vector.x = two.width / 2 + two.width / 2 + e / 2, vector.y = two.height / 2, Matter.Body.setPosition(bounds.right.entity, vector), Matter.Body.scale(bounds.right.entity, 1 / bounds.right.entity.scale.x, 1 / bounds.right.entity.scale.y), Matter.Body.scale(bounds.right.entity, e, i), bounds.right.entity.scale.x = e, bounds.right.entity.scale.y = i, vector.x = two.width / 2, vector.y = getFloorY() + e / 2, Matter.Body.setPosition(bounds.bottom.entity, vector), Matter.Body.scale(bounds.bottom.entity, 1 / bounds.bottom.entity.scale.x, 1 / bounds.bottom.entity.scale.y), Matter.Body.scale(bounds.bottom.entity, t, e), bounds.bottom.entity.scale.x = t, bounds.bottom.entity.scale.y = e, two.renderer.domElement.setAttribute("data-pixel-ratio", two.renderer.ratio);
...
// Breaks the bio sentence down into individual lowercase letters (instead
// of laying it out as wrapped words) and drops each one in at a random
// spot near the top of the canvas, at a reduced scale. Gravity and the
// existing wall/collision setup do the rest: letters fall and settle into
// a loose, rotated scatter with visible negative space between them.
...
// to jam Matter's solver into a locked pile that resisted gravity. Fitting
// every letter of the full sentence without overlap, at a size this large,
// needs noticeably more vertical room than one screen - so the grid runs
// upward past the visible top of the canvas as needed, and the extra rows
// simply fall into view and land on the rows below them, exactly like a
// stack toppling into a pile. (There is deliberately no ceiling wall this
// time, so that tall column of rows isn't blocked from existing above the
// frame; a vigorous fling could in theory send a letter off the top.)
function addLetters() {
    var raw = document.querySelector("#interactive__text").textContent
        , chars = raw.replace(/\s+/g, "").split("")
        , size = getFontSize()
        , cell = size * 1.3
        , margin = Math.max(cell / 2, bounds.thickness + cell / 2)
    ...
    for (var k = 0; k < chars.length; k++) {
        try {
            ...
            label.size = size, label.leading = size;
            var box = label.getBoundingClientRect(!0)
                , x = margin + cell / 2 + col * cell + (Math.random() - .5) * jitter
                , y = size + row * cell + (Math.random() - .5) * jitter;
            label.translation.y = 14, group.isLetter = !0, group.translation.x = x, group.translation.y = y;
            var outline = new Two.Rectangle(0, 0, box.width, box.height);
            outline.stroke = "rgb(green)", outline.noFill(), outline.opacity = .75, outline.visible = debug;
            var body = Matter.Bodies.rectangle(x, y, 1, 1, wordMaterial);
            Matter.Body.scale(body, box.width, box.height), Matter.Body.rotate(body, Math.random() * Math.PI * 2), body.scale = new Two.Vector(box.width, box.height), body.object = group, entities.push(body);
            group.text = label, group.rectangle = outline, group.entity = body, group.add(label, outline), two.add(group)
        } catch (err) {
...
function explode(e) {
    if (!e.object.isSymbol) {
        ...
        for (var a = 0; a < i.length; a++) {
            ...
            e = Matter.Bodies.rectangle(g, p, 1, 1, wordMaterial);
...
function injectSymbol(e) {
    ...
    var c = Math.random() * Math.PI / 16 - Math.PI / 32
        , h = Matter.Bodies.rectangle(s, o, 1, 1, wordMaterial);
...
function getFontSize() {
    ...
    return sizeScalar * (2 < t ? .14 * e : 1.75 < t && t < 2.01 ? .14 * e : 1.5 < t && t < 1.76 ? .12 * e : 1.25 < t && t < 1.51 ? .11 * e : 1 < t && t < 1.26 ? .105 * e : .9 < t && t < 1.01 ? .095 * e : .7 < t && t < .91 ? .085 * e : .6 < t && t < .71 ? .08 * e : .5 < t && t < .61 ? .075 * e : .45 < t && t < .51 ? .07 * e : .4 < t && t < .46 ? .065 * e : .35 < t && t < .41 ? .06 * e : .3 < t && t < .36 ? .055 * e : .25 < t && t < .31 ? .05 * e : .2 < t && t < .26 ? .045 * e : .1 < t && t < .11 ? .032 * e : .03 * e)
}
...
function startWhenFontsReady() {
    ...
    Promise.race([document.fonts.ready, timeout]).then(setup);
}
startWhenFontsReady();